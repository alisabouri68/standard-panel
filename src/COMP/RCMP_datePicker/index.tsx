...
function Index(dimention: Dimention) {
  ...
  useEffect(() => {
    if (dimention.defVal === "$today")
      setValue(moment().format(dimention.format));
    else setValue(dimention.defVal);
  }, [dimention.defVal]);
  ...
  return (
    <div className="border-2 w-full flex flex-col rounded-xl">
      ...
      <div className="p-4 w-full">
        <DateTimePicker
          ...
          onChange={(value: any) =>
            setValue(
              moment(new Date(value).toDateString()).format(
                dimention.format.replace("dd", "DD")
              )
            )
          }
          format={dimention.format}
          autoFocus={false}
          isCalendarOpen={false}
          required={dimention.required}
          calendarIcon={<Calendar size={24} color="currentColor" />}
    ...
    //         <Modal.Header>Date Picker</Modal.Header>
    //         <Modal.Body className='p-6 flex flex-none overflow-hidden h-96'>
    //             <div className=' w-full flex justify-start items-start'>
    //                 <DateTimePicker
    //                     value={value}
    //                     onChange={(value: any) => setValue(moment(new Date(val).toDateString()).format(dimention.format.replace("dd", "DD")))}
    //                     format={dimention.format}
    //                     autoFocus={true}
    //                     isCalendarOpen={true}
    //                     required={dimention.required}
