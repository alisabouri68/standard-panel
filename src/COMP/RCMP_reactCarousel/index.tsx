...
export const Carousel = forwardRef((userProps: CarouselProps, ref) => {
  ...
  useEffect(() => {
    if (typeof props?.target !== "number" || !props?.direction) return;

    slide(
      //@ts-ignore
      props?.direction === "right" ? SlideDirection.Right : SlideDirection.Left,
      props?.target
    );
  }, [props?.target, props?.direction]);
  ...
  return (
    <div
      ...
      className={`${styles["carousel-base"]} ${props.className}`}
    >
      {showArrow.left && (
